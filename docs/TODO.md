...
- [x] **접근성 도구 모음 (Accessibility Toolbar) 컴포넌트** ✅ 완료

  - [x] 플로팅 버튼 UI 구현
  - [x] 화면 확대/축소 기능 (50% ~ 200%)
  - [x] 고대비 모드 토글
  - [x] 폰트 크기 조절 (작게/보통/크게/아주크게)
  - [x] 줄 간격 조절
  - [x] 애니메이션 비활성화 옵션
  - [x] 설정 저장 (localStorage)
  - [x] 키보드 단축키 지원 (Alt + A)

- [x] **TTS (Text-to-Speech) 기능** ✅ 완료
...
- [x] **이미지 및 미디어 접근성** ✅ 완료
  ...
  - [x] 모든 이미지에 `alt` 속성 제공 ✅
    - [x] TestimonialCarousel: 프로필 이미지 alt 텍스트 개선 ("사용자 {name}의 프로필 사진")
    - [x] ExpertCard: 프로필 이미지 alt 텍스트 개선 ("전문가 {name}의 프로필 사진")
    - [x] RecommendationCard: 상품 이미지 alt 텍스트 개선 ("{product.name} 보조기기 이미지")
  - [x] 장식용 이미지는 `alt=""` 처리 ✅ (현재 프로젝트에 장식용 이미지 없음, 아이콘은 aria-hidden="true" 처리됨)
  - [x] 복잡한 이미지는 긴 설명 제공 ✅ (현재 이미지는 단순한 프로필/상품 이미지로 적절한 alt 텍스트 제공)
  - [x] 동영상 자막 제공 ✅ (현재 프로젝트에 동영상 없음)
  - [x] 오디오 트랜스크립트 제공 ✅ (현재 프로젝트에 오디오 없음)
